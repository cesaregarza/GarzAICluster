...
2. `citrus-dev/django-secrets` remains encrypted and unchanged, but an activated
   ...
   five database fields, and web additionally selects its exact dev webhook
   field. No process receives its payment API or production webhook fields.
...
- Web receives test API and publishable settings from the dedicated Secret,
  exactly one existing dev webhook field, and `STRIPE_WEBHOOK_SECRET_OWNER`.
  Billing and recurring consumers receive API authority only when enabled.
  Media, migrations, metrics, Redis, and media CronJobs receive no dedicated
  payment reference and no broad application Secret import.
- A semantic rollout revision changes pod templates without deriving anything
  from credential material.

This preparation created no Secret, credential, ciphertext, Application
...
3. Leave the encrypted dev `django-secrets` source unchanged. When the dedicated
   ...
   references and one exact dev webhook reference for web only.
...
3. Roll the dev workloads so the broad Secret projection is removed from every
   new pod. Future Jobs and CronJobs must render without payment settings unless
