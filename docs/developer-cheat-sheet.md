...
| Provision bot DB login | `BOT_DB_ADMIN_URL=... uv run python scripts/provision_bot_db.py <bot>` (auto-writes `secrets/bots/<bot>/db-secret.enc.yaml` + auto-encrypts when sops is installed). Argo decrypts at sync time via ksops. |
