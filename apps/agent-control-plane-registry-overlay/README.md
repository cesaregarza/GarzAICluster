...
The registry overlay and `agent-workloads` are manual-sync. A re-pin merge leaves the active registry and workers on their existing tuples until one scoped deployment activates both. Use `scripts/mandate_scoped_deploy.py` with the overlay and workers selected at the same confirmed GitOps SHA; the helper waits for full overlay hooks and health, then reconciles workers and runs fresh verification without another operator action. Selecting the overlay without workers is rejected. Do not sync the overlay independently for a re-pin: `previous_release` authorizes an old identity but does not let old code claim a new-release job. This sequence removes the unattended merge-to-sync gap; Kubernetes rollout and failure recovery remain bounded stages, not an atomic switch. See the scoped deployment command in `scripts/README.md`.

Before the first re-pin using this policy, confirm the live overlay Application has adopted the manual `syncPolicy` from `argocd/applications/agent-control-plane-registry-overlay.yaml`. A merged policy file alone is not proof of adoption.

After old pods have drained and live verification is complete, retire the `previousRelease` and `previous_release` tuple together and regenerate the committed render golden and grant-ownership outputs. Preserve the independent `hmacRollbackRelease` tuple.
